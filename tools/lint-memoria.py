...
# my-cv non ha un registro dei pendenti a tabella: le voci aperte stanno in tools/roadmap-items.yml,
# che ha un formato diverso e che roadmap.py gia' misura. Il controllo 2 resta quindi spento.
REGISTRO = ""
# Che cosa conta come codice in questo progetto: il sorgente LaTeX, la classe, gli script e gli
# strumenti. Un commit che li tocca senza voce e' un difetto, il resto un avviso.
CODICE = ("main.tex", "altacv.cls", "scripts/", "tools/")
# Data di adozione in my-cv: le voci precedenti non dichiaravano i file toccati, e la regola non
# si impone al passato.
ADOZIONE = "2026-09-24"
...
def main():
    ...
    for sha, data_c, soggetto, file in commit_dopo(data):
        codice = [f for f in file if f.startswith(CODICE)]
    ...
    if REGISTRO and os.path.exists(REGISTRO):
    ...
    for v in voci:
        m = RE_VOCE.match(v)
        if not m or m.group(1) < ADOZIONE:
