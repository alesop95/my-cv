...
.DESCRIPTION
  ...
  senza errori di "comando gia' definito".
  ...
  nelle build reali di questo progetto). Non usa latexmk perche' l'argomento iniettato non e' un
  ...
  file mantiene comunque la lingua, cosi' resta riconoscibile anche se estratto dalla cartella
  (es. condiviso via email). Senza componente oraria nel nome, rilanciare lo script piu' volte lo
...
try {
    foreach ($lang in $languages) {
        ...
        # solo il pdf: pdflatex li scrive sempre in $mainDir (la radice del progetto) perche' e'
