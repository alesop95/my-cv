...
Il passo immediatamente successivo, e non ancora fatto, è lo spostamento su Proton delle quattro cartelle di archivio, `Ongoing studies` compresa. Una decisione la precede e va presa prima di toccare qualunque configurazione: quale topologia deve avere il mirror dopo lo spostamento, cioè se `C:\Scripts\folder-sync-watcher` continua a sincronizzare in bidirezionale con la OneDrive aziendale dalla nuova posizione o in una direzione sola. È stabilito che il watcher va riorientato sui percorsi nuovi e che deve prendere la versione anonimizzata; non è stabilito il resto.
