...
Dipendenza fuori repository, registrata il 2026-09-07 e non ancora toccata. Il watcher sincronizza in bidirezionale la cartella `CYBERSECURITY, BC, governance`, che vive sull'SSD etichettato `T7` sotto `googleDrive_sync\Portfolio and ongoing studies\🛠️ Ongoing studies\Cybersec, BC, IT governance, DevOps, Programming, Networking`, con la OneDrive aziendale in `Documenti - IT\Cybersec & IT Governance\_ 🧰 Resources`. Risolve i conflitti per data con la politica `newest`, e sorveglia la cartella attraverso un `subst` sull'unità `A:` per stare sotto il limite di 260 caratteri di Windows. È fermo dal 21 gennaio 2026, verificato il 2026-09-08: nessun task pianificato, nessun processo, `config.json` non toccato da quella data.

Cosa la rompe: uno spostamento delle cartelle di archivio eseguito senza aggiornare il watcher, che continuerebbe a sorvegliare un percorso morto senza dirlo. Cosa fare: aggiornare il watcher nella stessa sessione in cui si spostano le cartelle, mai in una successiva.

Topologia decisa il 2026-09-08 per il dopo-spostamento: il watcher resta bidirezionale con la stessa politica e si allinea alla stessa cartella nella sua nuova posizione dentro l'albero locale di Proton. Le tre condizioni che quella scelta richiede, cioè che il sottoalbero sia reale in locale e non a segnaposto, con il pin fatto dal watcher stesso, che il `subst` sia conservato perché i percorsi interni arrivano a 157 caratteri, e che al watcher si aggiunga una base esplicita in configurazione perché oggi il bersaglio del `subst` lo deriva dall'etichetta del volume dentro il codice, stanno nella Fase 7 di `roadmap.md` con le misure che le motivano.
...
Rischio noto e non presidiato, indipendente dalla topologia: il mirror è continuo mentre l'anonimizzazione di ADR-011 è stata un intervento una volta sola su quattro file, quindi un documento aziendale nuovo che finisse nella cartella OneDrive si propagherebbe su Proton non anonimizzato. Chiuderlo richiederebbe un controllo sui marcatori aziendali dentro il watcher, prima di propagare, che oggi non esiste.
