...
Struttura delle cartelle fissata il 2026-09-07, e da non riorganizzare più: quattro cartelle sotto `My files`, cioè `Certifications`, `Portfolio`, `Education studies` e `Research projects` con `Thesis` al suo interno. Il censimento del disco del 2026-09-10 dice però che le cartelle di primo livello sono sei e non quattro: oltre alle quattro documentate ci sono `Portfolio and ongoing studies`, aggiunta il 2026-09-09 per l'archivio della Fase 7, e `Sugimori Pokémon Gen1-9 DLC3 Organized`, 1857 file per 808,9 MB, che non è mai stata censita qui e non ha alcun rapporto con il CV. L'aggiunta di cartelle non contraddice il vincolo: quel vincolo protegge i dodici file già linkati dal CV, che non si muovono, e non impedisce di creare cartelle nuove accanto a loro. Quel sottoalbero è tenuto a solo online dal 2026-09-10, perché la macchina è aziendale e in locale deve materializzarsi soltanto il materiale di studio anonimizzato per ADR-011. Sostituisce la struttura a sei cartelle solo raccomandata di prima, che non è mai stata realizzata. Si condivide sempre il singolo documento necessario, mai la cartella radice. Il vincolo di non riorganizzare non è una preferenza di ordine: un link condiviso di Proton è legato alla copia specifica del file, quindi spostare la copia rompe il link, e la riorganizzazione del 2026-09-07 ha effettivamente invalidato in un colpo tutti e dodici i link del perimetro, che sono stati rigenerati e riverificati uno per uno.
...
Dipendenza fuori repository, registrata il 2026-09-07 e ripuntata il 2026-09-14 con il microstep 4 della Fase 7. Il watcher sincronizza in bidirezionale la cartella `CYBERSECURITY, BC, governance` con la OneDrive aziendale in `Documenti - IT\Cybersec & IT Governance\_ 🧰 Resources`. La sorgente non è più sull'SSD `T7`: vive sotto `C:\Users\Utente\Proton Drive\alesop95\My files\Portfolio and ongoing studies\🛠️ Ongoing studies\Cybersec, BC, IT governance, DevOps, Programming, Networking`, dichiarata in `config.json` con `folders.source_base` invece che per etichetta di volume. Risolve i conflitti per data con la politica `newest`, ancora e libera il perimetro locale con `folder_sync_watcher/pin.py` (ADR-008 e ADR-009 di quel repository), e sorveglia la cartella attraverso un `subst` sull'unità `A:` per stare sotto il limite di 260 caratteri di Windows. Non è più fermo: avviato per la prima volta il 2026-09-14, verificato con due run manuali senza errori residui.

Cosa la rompe: uno spostamento delle cartelle di archivio eseguito senza aggiornare il watcher, che continuerebbe a sorvegliare un percorso morto senza dirlo. Cosa fare: aggiornare il watcher nella stessa sessione in cui si spostano le cartelle, mai in una successiva. Verificato per davvero il 2026-09-14: questo è esattamente cosa fatto per il microstep 4.

Vincolo di perimetro, che è il più importante da rispettare quando si tocca questa configurazione: la macchina è aziendale, quindi l'unico oggetto del sync locale sono i file di studio anonimizzati per ADR-011, e tutto il resto di Proton resta solo in cloud. Il pin va quindi dichiarato per quel sottoalbero e non attivato in blocco, perché la scelta di cosa scende in locale qui è una scelta di riservatezza: materiale personale non anonimizzato non si materializza su un computer aziendale.

Rischio noto e non presidiato, materializzato per la prima volta il 2026-09-14: due cartelle con lo stesso contenuto ma nomi diversi sui due lati (`NIST Cybersecurity Framework - CSF` con spazio, `-CSF` senza) non sono state riconciliate come la stessa cartella, e il primo avvio reale ha reintrodotto su Proton uno screenshot anonimizzato da ADR-011 sotto il suo nome originale mai anonimizzato. Chiuso a mano lo stesso giorno (cartella col nome sbagliato cancellata da entrambi i lati, verificato anche in cloud), non nel codice: ADR-010 di `folder-sync-watcher` documenta il gap e le opzioni scartate. Chi ripunta il watcher su una coppia nuova deve verificare a mano che i nomi di primo livello coincidano esattamente sui due lati, prima del primo avvio, specialmente quando un lato ha subito un'anonimizzazione che l'altro non ha mai visto. Resta valido anche il rischio più generale, indipendente da questo episodio: un documento aziendale nuovo che finisse nella cartella OneDrive si propagherebbe su Proton non anonimizzato, perché il watcher non ha alcun controllo sui marcatori aziendali prima di propagare.

## lettore-doc (`E:\lettore-doc`)

Dipendenza fuori repository per il secondo obiettivo della Fase 7: `lettore-doc` legge `Ongoing studies` e il resto del portfolio come sorgente, ne estrae una tassonomia di competenze attraverso i suoi gate di sanificazione, e la pubblica su `skills-repo`, che il CV linka già con sei link. `my-cv` non chiama la pipeline, la dichiara come sorgente e basta: l'esecuzione, con tutti i suoi passi umani (selezione dei documenti, sessione interattiva `/graphify .`, revisione del diff), è lavoro di quel repository.

Cosa la rompe: uno spostamento delle cartelle sorgente eseguito senza aggiornare `sources.yml`/le variabili d'ambiente `LETTERDOC_SOURCE_*`, che lascerebbe la sorgente puntata su un percorso morto senza dirlo finché qualcuno non prova a lanciare un ciclo. Verificato per davvero il 2026-09-14: `LETTERDOC_SOURCE_PORTFOLIO` puntava ancora al percorso su `J:` rimosso dal microstep 1C del 2026-09-10, senza che nessuno se ne accorgesse perché nessun ciclo l'aveva ancora usata. Ripuntata alla radice Proton che oggi contiene tutte e cinque le cartelle del portfolio.

Stato al 2026-09-14: sorgente dichiarata e funzionante, zero cicli mai eseguiti su questo perimetro. Dettaglio operativo nella memoria di `lettore-doc`, non qui.
